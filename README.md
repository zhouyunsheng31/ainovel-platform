...
| 11. 集成测试 | ⚠️ 部分 | 章纲LLM真实调用验证通过；粗纲/大纲/世界纲待多章节数据验证 |
| 12. PRD验收 | ⚠️ 待联调 | 功能代码已实现，待前后端真实联调后做最终验收 |
...
| 模块 | 状态 | 说明 |
|------|------|------|
| 粗纲生成 | ⚠️ | 需10份章纲输入，测试文件仅1章 |
| 大纲生成 | ⚠️ | 需10份粗纲输入，依赖粗纲 |
| 世界纲生成 | ⚠️ | 需多份大纲输入，依赖大纲 |

### ⚠️ PRD验收对照（代码级验收通过，待真实联调后最终确认）
...
| 模块 | 优先级 | 说明 |
|------|--------|------|
| **前端启动修复** | P0 | 修复页面打开即报错问题 |
| **密钥清理** | P0 | 清除仓库中真实密钥 |
| **工作流测试** | P1 | LangGraph工作流纳入pytest |
| **前后端联调** | P1 | 跑通上传到详情页主链路 |
| **APK最终构建** | P1 | 需在PC端Android Studio中验证 |
...
> - ✅ 质量保障（98个单元测试 + Ruff lint + Spectral契约验证）
> - ✅ CI/CD流水线（GitHub Actions自动化）
> - ✅ Android打包准备（Capacitor + Gradle配置完整）
> - ✅ 门禁命令标准化（gate-check.ps1 + npm快捷命令）
> - ✅ APK构建成功（app-debug.apk 5.6MB）
> - ✅ 真实 API 联调 + WebSocket 联调 + 主链路打通
> - ✅ PRD 功能回归（8/8 通过）+ 非功能回归验收
> - ✅ 多章节与多层级数据回归（26个测试用例）
> - ✅ 真实 LLM 冒烟测试（3/3 通过）
> - ✅ 错误码统一 + 结构化日志增强
>
> **待发布收尾（详见 [后续开发规划](docs/后续开发规划.md)）：**
> - 🔧 Python 3.14 兼容告警治理
> - 🔧 Android SDK 完整环境构建验证
> - 🔧 性能与并发测试
>
> **项目整体状态**：按 `docs/后续开发规划.md` 推进至发布准备阶段 (P6)，**文档与清单冻结已完成**，详见 `docs/发布清单.md` 和 `docs/已知问题.md`
